...
import src.extract
import src.extract.from_db
import src.transform.by_aggregation
import src.transform.by_adding_attribute
...
async def overview(request: inputData):
    ...
    dict_attribute = src.extract.from_db.by_attribute(db_file, attribute)
    df_attribute = pd.DataFrame(dict_attribute)
    groupby_attribute = src.transform.by_aggregation.sum_by_attribute(attribute[:-2], df_attribute)
    result = src.transform.by_adding_attribute.last_price(attribute[:-2], groupby_attribute)
